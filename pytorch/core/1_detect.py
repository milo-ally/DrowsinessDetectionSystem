...
# 核心推理函数（无修改）
def face_detect(model, img, device, img_size=640, conf_thres=0.6, iou_thres=0.5):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 人脸检测
        frame_det = face_detect(model, frame, device, conf_thres=CONF_THRES)
        
        # ========== 新增：绘制帧数到画面左上角 ==========
        h, w = frame_det.shape[:2]
        tl = round(0.002 * (h + w) / 2) + 1
        tf = max(tl - 1, 1)
        cv2.putText(frame_det, f'FPS: {fps:.1f}', (10, 30), 0, tl/3, [0, 255, 255], thickness=tf, lineType=cv2.LINE_AA)

        # 显示结果
        cv2.imshow('YOLOv5 Face Detection (Camera)', frame_det)
