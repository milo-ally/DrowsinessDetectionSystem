...
def generate_rknn(onnx_path, input_size_list, target_platform="rk3588"):
    ...
    if rknn.load_onnx(
        model=onnx_path, 
        input_size_list=input_size_list) != 0:
...
# 图像预处理
def preprocess_image(img_frame, img_size=640):
    img0 = img_frame.copy() # "type(img_frame) = type(cv2.imread(img_path))" 
    img = img0.copy()
    img_letter, ratio, pad = letterbox(img, new_shape=(img_size, img_size)) # letterbox缩放补边
...
def face_detect_rknn(
    ...
    #  图像预处理
    img_input, img_letter, img0, (ratio, pad) = preprocess_image(img_frame, img_size)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # 人脸检测推理
            det_frame = face_detect_rknn(rknn_model, frame, img_size=IMG_SIZE, 
                                         conf_thres=CONF_THRES, iou_thres=IOU_THRES)
            # 计算并绘制FPS
            fps = 1.0 / (time.time() - start) if (time.time() - start) > 0 else 0.0
            cv2.putText(det_frame, f"FPS: {fps:.1f}", (10, 40), 
